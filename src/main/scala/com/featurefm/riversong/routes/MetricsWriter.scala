...
class MetricsWriter(registry: MetricRegistry) {
  ...
  def getMetrics(includeJvm: Boolean, pattern: Option[String]): JValue = {
    if (includeJvm) {
      "system" ->
        ("jvm" -> getVmMetrics(pattern)) ~
          ("metrics" -> getCustomMetrics(pattern))
    }
    else {
      "system" -> ("metrics" -> getCustomMetrics(pattern))
    }
  }
